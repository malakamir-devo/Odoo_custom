...
#-------------------------------------------------------------------------------
# Make a new file:
# sudo nano odoo_install_akuma.sh
# Place this content in it and then make the file executable:
# sudo chmod +x odoo_install_akuma.sh
# Execute the script to install Odoo:
# ./odoo_install_akuma.sh
################################################################################
...
# Prompt for port number and set default to 8069 if no input is given
read -p "Enter Odoo port number (default 8069): " OE_PORT
OE_PORT=${OE_PORT:-8069}  # If no input, default to 8069
...
sudo apt-get install libldap2-dev libsasl2-dev libssl-dev -y
...
sudo -H pip3 install --break-system-packages -r $OE_HOME_EXT/requirements.txt
sudo -H pip3 install --break-system-packages -r $OE_HOME_EXT/requirements.txt
pip3 install --break-system-packages -r $OE_HOME_EXT/requirements.txt
...
if [ $IS_ENTERPRISE = "True" ]; then
    ...
    mkdir /${OE_HOME}/${OE_CONFIG}/custom_addons
    sudo su root -c "printf 'addons_path=${OE_HOME_EXT}/addons,${OE_HOME}/${OE_HOME}-server/odoo/addons,/${OE_HOME}/${OE_CONFIG}/custom_addons\n' >> /etc/${OE_CONFIG}.conf"
fi
...
# Step 3: Check status of the service
echo -e "* Checking Odoo service status"
sudo systemctl status $OE_CONFIG.service
...
echo "Logfile name: /var/log/$OE_USER/${OE_CONFIG}.log"
echo "User PostgreSQL: $OE_USER"
echo "Code location: $OE_USER"
echo "Addons folder: $OE_USER/$OE_CONFIG/addons/,$OE_USER/$OE_CONFIG/odoo/addons/"
echo "Password superadmin (database): $OE_SUPERADMIN"
echo "Start Odoo service: sudo service $OE_CONFIG start"
echo "Stop Odoo service: sudo service $OE_CONFIG stop"
echo "Restart Odoo service: sudo service $OE_CONFIG restart"
echo "Live Odoo Log: sudo tail -f /var/log/$OE_USER/${OE_CONFIG}.log"
if [ $INSTALL_NGINX = "True" ]; then
